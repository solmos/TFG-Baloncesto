...
\begin{minipage}{.5\textwidth}
  \centering
  \captionsetup{width=.9\linewidth}
  \includegraphics[width=.8\linewidth]{BoxScore2022.jpg}
...
Este tipo de analisis se inicializaron en 2007 para la Euroliga (principal competición europea de baloncesto) y en 2012 para la Eurocopa (competición internacional de segundo nivel).
...
El Short-Charts, proporciona un output visual muy fácil de interpretar, ya que es se parece a un mapa de calor y, por lo tanto, podemos observar de una manera muy rápida des de que zonas de la pista es más efectivo el equipo y/o el jugador.
...
<<Datos echo=TRUE, eval=TRUE, include=TRUE>>=
...
df_pbp <- pbp_2018_fixed %>% select(season, game_code, quarter, points_home, points_away, matches("_player[1-5]"))
df_pbp$PM <- df_pbp$points_home - df_pbp$points_away
@
